...
def built_mol_graph_data_and_save(
    ...
    # if path doesnt exist, then create this path / file
    print('Save the molecule graph data to {}'.format(save_g_group_path))
    #if not os.path.exists(save_g_group_path):
    #    os.makedirs(os.path.dirname(save_g_group_path))
...
def load_graph_from_csv_bin_for_splited(
    ...
    smiles = data.smiles.values   # the smiles strings
    group = data.group.to_list()  # the group of the molecule (training, validation, test)
    ...
    homog, detailed_information = load_graphs(bin_path)  # load the graph from the bin file and loast the labels
    ...
    if smask_path is None:
        ...
        smask = np.load(smask_path, allow_pickle=True)   # the smask for a molecule will be a list of integers. I believe that each integer represents an atom that will be masked (?)
    ...
    for i in train_index:
        molecule = [smiles[i], homog[i], labels[i], smask[i], sub_name[i]]   # this is how we code a molecule and its information. We have: smiles string, the graph representation, the label, the substructure mask, sub_name
    ...
    print("Train: ", len(train_set), " --- Validation: ", len(val_set), " --- Test: ", len(test_set), " --- TASK NUMBER: ", task_number)
