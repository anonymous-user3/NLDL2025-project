...
# this class is used to calculate the embedding of the whole molecule
# we can use a substructure mask (smask) when calling this class to get the masked embedding (I think)
...
class RGCNLayer(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, num_rels=65, activation=F.relu, loop=False,
        ...
        self.graph_conv_layer = RelGraphConv(in_feats, out_feats, num_rels=num_rels, regularizer='basis',
                                             num_bases=2, bias=True, activation=activation,
                                             self_loop=loop, dropout=rgcn_drop_out)
...
class BaseGNN(nn.Module):
    ...
    def __init__(self, gnn_rgcn_out_feats, ffn_hidden_feats, ffn_dropout=0.25, classification=True):
        ...
        self.readout = WeightAndSum(gnn_rgcn_out_feats)   # this calculate the embedding! (here it comes into place if there is a mask or not)
...
# function for evaluating the model (with substructure mask)
def run_an_eval_epoch(args, model, data_loader, loss_criterion, out_path, seed=0):
    ...
    with th.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            smiles, rgcn_bg, labels, smask_idx, sub_name = batch_data  # smiles string, graph representation of molecule, label, substructure mask, subgroupname
            rgcn_bg = rgcn_bg.to(args['device'])
            labels = labels.unsqueeze(dim=1).float().to(args['device'])
    ...
    if out_path is not None:
        ...
        out_path_file = out_path + '_smask_index.npy'
        if not os.path.exists(os.path.dirname(out_path_file)):
            os.makedirs(os.path.dirname(out_path_file))
        #np.save(out_path_file, smask_idx_list)   # this caused an error (ValueError: setting an array element with a sequence. The requested array has an inhomogeneous shape after 1 dimensions. The detected shape was (237,) + inhomogeneous part.)
        # workaround: turn the smask_idx_list into a np array with object dtype
        smask_idx_list = np.asarray(smask_idx_list, dtype=object)
        np.save(out_path_file, smask_idx_list)
