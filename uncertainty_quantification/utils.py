...
def sub_attribution_visualize(smiles, atom_attribution, bond_attribution, ring_attribution, atom_list=None, bond_list=None,
    ...
    for i in atom_list:
        ...
        attribution_scores[i] = [atom_attribution[atom_list.index(i)]]
        atomrads[atom_list.index(i)] = 0.2
        
        # NEW
        attr = atom_attribution[atom_list.index(i)]
        if attr not in previous:
            mol.GetAtomWithIdx(i).SetProp('atomNote', str(round(attr, 4)))
...
#     drawer.SetFontSize(1)
    ...
    dos.annotationFontScale = 0.5 # NEW
    ...
    # now draw the molecule, with highlights:
    # drawer.DrawMoleculeWithHighlights(mol, smiles, highlight_atom_colors, highlight_bond_colors, atomrads, widthmults)
    drawer.DrawMoleculeWithHighlights(mol, sub_type, highlight_atom_colors, highlight_bond_colors, atomrads, widthmults)
